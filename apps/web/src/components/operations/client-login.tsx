...
async function clientLogin(
  ...
  const response = await fetch(`${env.NEXT_PUBLIC_API_URL}/auth/client/login`, {
  ...
  const payload = (await response.json()) as { user: ClientSessionUser };
  return payload.user;
}
...
export function ClientLogin() {
  ...
  async function submitLogin(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    setError("");
    setStatus("Menghubungi backend client...");
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Login client tidak dapat diproses.",
      );
    } finally {
      if (!navigating) {
        setSubmitting(false);
  ...
  return (
    <div className="mx-auto max-w-xl border border-white/12 bg-[#181815] p-6 md:p-8">
      ...
      <h2 className="mt-5 font-serif text-4xl">Login client.</h2>
      <p className="mt-4 text-sm leading-6 text-white/55">
        Masuk untuk meninjau draft, mengirim revisi, dan approve publish.
      </p>
